...
export default function ReportForm({ devices, isLoggedIn, preselectedGameId, preselectedGameName, preselectedDeviceId }: Props) {
  ...
  if (!isLoggedIn) {
    return (
      <div className="text-center py-12">
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-lg bg-[var(--color-elevated)]">
          <svg className="h-8 w-8 text-[var(--color-text-dim)]" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
          </svg>
        </div>
        <h3 className="text-lg font-semibold text-white">Sign in to submit a report</h3>
        <p className="mt-2 text-sm text-[var(--color-text-2)]">You need to be signed in to submit performance reports.</p>
        <a href="/auth/login?redirect=/report/new" className="mt-4 inline-flex items-center gap-2 rounded-lg bg-[#D4FF00] px-4 py-2 text-sm font-medium text-[#0a0a0a] hover:bg-[#D4FF00] transition-colors">
          Sign in
        </a>
      </div>
    );
  ...
  if (submitted) {
    return (
      <div className="text-center py-12">
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-lg bg-[#D4FF00]/20">
          <svg className="h-8 w-8 text-[#D4FF00]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
          </svg>
        </div>
        <h3 className="text-lg font-semibold text-white">Report submitted!</h3>
        <p className="mt-2 text-sm text-[var(--color-text-2)]">Thank you for contributing to the community.</p>
        <div className="mt-6 flex justify-center gap-3">
          <a href={`/games/${data.gameSlug}`} className="rounded-lg bg-[var(--color-elevated)] px-4 py-2 text-sm text-gray-300 hover:bg-[var(--color-border)] transition-colors">
            View game
          </a>
          <button onClick={() => { setSubmitted(null); setStep(1); setData(INITIAL); }} className="rounded-lg bg-[#D4FF00] px-4 py-2 text-sm font-medium text-[#0a0a0a] hover:bg-[#D4FF00] transition-colors">
            Submit another
          </button>
        </div>
      </div>
    ...
    <div>
      ...
      <div className="mb-8">
          ...
          {['Game & Device', 'Settings & FPS', 'Overall', 'Review'].map((label, i) => (
            <button
              ...
              className={`text-xs font-medium transition-colors ${
                i + 1 === step ? 'text-[#D4FF00]' : i + 1 < step ? 'text-[var(--color-text-2)] cursor-pointer hover:text-white' : 'text-gray-600'
              }`}
            >
              {label}
            </button>
        ...
        <div className="h-1 rounded-full bg-[var(--color-elevated)]">
          <div className="h-full rounded-full bg-[#D4FF00] transition-all duration-300" style={{ width: `${(step / 4) * 100}%` }} />
        </div>
      </div>
      ...
      <div className="mt-8 flex justify-between">
        <button
          ...
          disabled={step === 1}
          className="rounded-lg border border-[var(--color-border)] px-4 py-2 text-sm text-gray-300 hover:border-[var(--color-text-dim)] hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
        >
          Back
        </button>
        {step < 4 ? (
          <button
            onClick={() => setStep(s => s + 1)}
            disabled={!canAdvance()}
            className="rounded-lg bg-[#D4FF00] px-6 py-2 text-sm font-medium text-[#0a0a0a] hover:bg-[#D4FF00] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Continue
          </button>
        ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {data.gameId ? (
          <div className="flex items-center gap-3 rounded-lg border border-[#D4FF00]/30 bg-[#D4FF00]/5 p-3">
            <span className="text-sm text-white font-medium flex-1">{data.gameName}</span>
            <button onClick={() => update({ gameId: '', gameName: '', gameSlug: '' })} className="text-xs text-[var(--color-text-2)] hover:text-white">
              Change
            </button>
          </div>
        ) : (
          <div className="relative">
            <input
              ...
              className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] py-2.5 px-4 text-sm text-white placeholder-[var(--color-text-dim)] focus:border-[#D4FF00] focus:outline-none focus:ring-1 focus:ring-[#D4FF00]"
              autoFocus
            />
            {results.length > 0 && (
              <div className="absolute z-10 mt-1 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-raised)] shadow-xl overflow-hidden">
                {results.map(hit => (
                  <button
                    key={hit.id}
                    onClick={() => {
                    ...
                    className="flex items-center gap-3 w-full px-3 py-2.5 text-left hover:bg-[var(--color-elevated)] transition-colors"
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      {hit.genres && <p className="text-xs text-[var(--color-text-dim)]">{hit.genres.slice(0, 2).join(', ')}</p>}
                    </div>
                  </button>
                ))}
              </div>
            ...
            <button
              ...
              className={`rounded-lg border px-3 py-2.5 text-sm text-left transition-colors ${
                data.deviceId === d.id
                  ...
                  : 'border-[var(--color-border)] bg-[var(--color-elevated)] text-gray-300 hover:border-[var(--color-text-dim)]'
              }`}
            >
              <span className="font-medium">{d.name}</span>
              <span className="block text-xs text-[var(--color-text-dim)] mt-0.5">{d.manufacturer}</span>
            </button>
          ))}
        </div>
      </div>
...
function Step2({ data, update }: { data: ReportData; update: (p: Partial<ReportData>) => void }) {
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-semibold text-[var(--color-text-2)]">Performance</h3>
      ...
      <hr className="border-[var(--color-elevated)]" />
      <h3 className="text-sm font-semibold text-[var(--color-text-2)]">Graphics Settings</h3>
      ...
      <div className="flex items-center gap-3">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={data.fsrEnabled} onChange={e => update({ fsrEnabled: e.target.checked })}
            className="rounded border-gray-600 bg-[var(--color-elevated)] text-[#D4FF00] focus:ring-[#D4FF00] focus:ring-offset-0" />
        ...
        {data.fsrEnabled && (
          <select value={data.fsrMode} onChange={e => update({ fsrMode: e.target.value })}
            className="rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] px-3 py-1.5 text-sm text-gray-300 focus:border-[#D4FF00] focus:outline-none">
      ...
      <hr className="border-[var(--color-elevated)]" />
      <h3 className="text-sm font-semibold text-[var(--color-text-2)]">Power</h3>
...
function Step3({ data, update }: { data: ReportData; update: (p: Partial<ReportData>) => void }) {
  ...
  const ratingColors: Record<string, string> = {
    copper: 'border-[#D4FF00] bg-[#D4FF00]/10 text-[#D4FF00]',
            ...
            <button
              ...
              className={`w-full rounded-lg border px-4 py-3 text-left transition-colors ${
                data.overallRating === r.value
                  ...
                  : 'border-[var(--color-border)] bg-[var(--color-elevated)] hover:border-[var(--color-text-dim)]'
              }`}
            >
              <span className={`font-medium ${data.overallRating === r.value ? '' : 'text-gray-300'}`}>{r.label}</span>
              <span className={`block text-xs mt-0.5 ${data.overallRating === r.value ? 'opacity-80' : 'text-[var(--color-text-dim)]'}`}>{r.desc}</span>
            </button>
          ))}
        </div>
      </div>
...
function Step4({ data, update }: { data: ReportData; update: (p: Partial<ReportData>) => void }) {
  const ratingEmoji: Record<string, string> = {
    excellent: 'text-[#D4FF00]', good: 'text-green-400', fair: 'text-yellow-400',
  ...
  return (
    <div className="space-y-6">
      {/* Summary */}
      <div className="rounded-lg border border-[var(--color-elevated)] bg-[var(--color-elevated)]/50 p-4 space-y-2">
        <div className="flex justify-between text-sm">
          <span className="text-[var(--color-text-dim)]">Game</span>
  ...
  return (
    <div className="mt-2">
      ...
      >
        ...
        {count > 0 && (
          <span className="rounded-full bg-[#D4FF00]/20 px-2 py-0.5 text-xs text-[#D4FF00]">{count}</span>
        )}
      </button>
          ...
          {settings.map((s, i) => (
            <div key={i} className="flex items-center gap-2">
              <input
                ...
                maxLength={50}
                className="flex-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] py-2 px-3 text-sm text-white placeholder-[var(--color-text-dim)] focus:border-[#D4FF00] focus:outline-none"
              />
              <input
                type="text"
                value={s.value}
                ...
                className="flex-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] py-2 px-3 text-sm text-white placeholder-[var(--color-text-dim)] focus:border-[#D4FF00] focus:outline-none"
              />
              <button type="button" onClick={() => removeRow(i)} className="text-[var(--color-text-dim)] hover:text-red-400 transition-colors p-1">
...
function Field({ label, value, onChange, type = 'text', placeholder }: {
  ...
  return (
    <div>
      <label className="block text-xs font-medium text-[var(--color-text-2)] mb-1.5">{label}</label>
      <input
        type={type}
        value={value}
        ...
        className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] py-2 px-3 text-sm text-white placeholder-[var(--color-text-dim)] focus:border-[#D4FF00] focus:outline-none focus:ring-1 focus:ring-[#D4FF00]"
      />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] py-2 px-3 text-sm text-gray-300 focus:border-[#D4FF00] focus:outline-none"
      >
        {options.map(([val, lbl]) => (
