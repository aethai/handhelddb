...
export default function SteamImport({ currentUserId: _userId }: Props) {
  ...
  return (
    <div className="rounded-xl border border-[#3A3D45]/50 bg-[#16181D]/60 backdrop-blur-sm">
      {/* Header */}
      <div className="flex items-center gap-3 border-b border-[#2A2D35] px-5 py-4">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#2A2D35]">
          {/* Steam icon */}
      ...
      <div className="px-5 py-4">
        ...
        {phase !== 'success' && (
          <div className="space-y-3">
            <div>
              <label htmlFor="steam-id-input" className="mb-1.5 block text-xs font-medium text-[#9CA3AF]">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg border border-[#3A3D45] bg-[#2A2D35]/80 px-3 py-2 text-sm text-white placeholder-gray-600 focus:border-[#60A5FA] focus:outline-none focus:ring-1 focus:ring-[#60A5FA] transition-colors disabled:opacity-50"
                />
                <button
                  onClick={handleImport}
                  disabled={phase === 'loading' || !input.trim()}
                  className="inline-flex items-center gap-2 rounded-lg bg-[#60A5FA] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#60A5FA] focus:outline-none focus:ring-2 focus:ring-[#60A5FA] focus:ring-offset-2 focus:ring-offset-[#16181D] disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {phase === 'loading' ? (
                    <>
                      <svg
            ...
            {phase === 'loading' && (
              <div className="flex items-center gap-2 rounded-lg border border-[#3A3D45]/50 bg-[#2A2D35]/40 px-3 py-2.5">
        ...
        {phase === 'success' && result && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg bg-[#2A2D35]/60 p-3 text-center">
                <p className="text-lg font-bold text-white">{result.totalOwned}</p>
                <p className="text-[11px] text-[#6B7280]">Games Owned</p>
              </div>
              <div className="rounded-lg bg-[#2A2D35]/60 p-3 text-center">
                <p className="text-lg font-bold text-[#60A5FA]">{result.matched}</p>
                <p className="text-[11px] text-[#6B7280]">Matched</p>
              </div>
              <div className="rounded-lg bg-[#2A2D35]/60 p-3 text-center">
                <p className="text-lg font-bold text-[#9CA3AF]">{result.totalOwned - result.matched}</p>
                <p className="text-[11px] text-[#6B7280]">Not in DB</p>
              </div>
            </div>
            ...
            {result.games.length > 0 && (
              <div>
                <h4 className="mb-2 text-xs font-medium text-[#9CA3AF]">
                  Matched Games ({result.matched})
                </h4>
                <div className="max-h-80 overflow-y-auto rounded-lg border border-[#2A2D35] divide-y divide-[#2A2D35]/80">
                  {gamesToShow.map((game) => (
                    <a
                      key={game.id}
                      href={`/games/${game.slug}`}
                      className="flex items-center gap-3 px-3 py-2 hover:bg-[#2A2D35]/60 transition-colors"
                    >
                      {game.headerImage ? (
                        <img
                        ...
                        <div className="flex h-8 w-14 flex-shrink-0 items-center justify-center rounded bg-[#2A2D35]">
                          <svg
                ...
                {hasMore && (
                  <button
                    ...
                    className="mt-2 w-full rounded-lg border border-[#2A2D35] py-1.5 text-xs text-[#9CA3AF] hover:border-[#3A3D45] hover:text-gray-300 transition-colors"
                  >
                    {showAllGames
                      ? 'Show less'
            ...
            {result.matched === 0 && (
              <div className="rounded-lg border border-[#3A3D45]/50 bg-[#2A2D35]/40 px-3 py-3 text-center">
            ...
            <button
              onClick={handleReset}
              className="w-full rounded-lg border border-[#3A3D45] py-2 text-xs font-medium text-[#9CA3AF] hover:border-[#4B5563] hover:text-gray-300 transition-colors"
            >
