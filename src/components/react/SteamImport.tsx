...
export default function SteamImport({ currentUserId: _userId }: Props) {
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border)]/50 bg-[var(--color-raised)]/60 backdrop-blur-sm">
      {/* Header */}
      <div className="flex items-center gap-3 border-b border-[var(--color-elevated)] px-5 py-4">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--color-elevated)]">
          {/* Steam icon */}
      ...
      <div className="px-5 py-4">
        ...
        {phase !== 'success' && (
          <div className="space-y-3">
            <div>
              <label htmlFor="steam-id-input" className="mb-1.5 block text-xs font-medium text-[var(--color-text-2)]">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)]/80 px-3 py-2 text-sm text-white placeholder-gray-600 focus:border-[#D4FF00] focus:outline-none focus:ring-1 focus:ring-[#D4FF00] transition-colors disabled:opacity-50"
                />
                <button
                  onClick={handleImport}
                  disabled={phase === 'loading' || !input.trim()}
                  className="inline-flex items-center gap-2 rounded-lg bg-[#D4FF00] px-4 py-2 text-sm font-medium text-[#0a0a0a] transition-colors hover:bg-[#D4FF00] focus:outline-none focus:ring-2 focus:ring-[#D4FF00] focus:ring-offset-2 focus:ring-offset-[var(--color-raised)] disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {phase === 'loading' ? (
                    <>
                      <svg
            ...
            {phase === 'loading' && (
              <div className="flex items-center gap-2 rounded-lg border border-[var(--color-border)]/50 bg-[var(--color-elevated)]/40 px-3 py-2.5">
                <div className="h-2 w-2 animate-pulse rounded-full bg-[#D4FF00]" />
            ...
            <p className="text-[11px] text-gray-600">
              ...
              <a
                ...
                className="text-[#D4FF00]/70 hover:text-[#D4FF00] transition-colors"
              >
        ...
        {phase === 'success' && result && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg bg-[var(--color-elevated)]/60 p-3 text-center">
                <p className="text-lg font-bold text-white">{result.totalOwned}</p>
                <p className="text-[11px] text-[var(--color-text-dim)]">Games Owned</p>
              </div>
              <div className="rounded-lg bg-[var(--color-elevated)]/60 p-3 text-center">
                <p className="text-lg font-bold text-[#D4FF00]">{result.matched}</p>
                <p className="text-[11px] text-[var(--color-text-dim)]">Matched</p>
              </div>
              <div className="rounded-lg bg-[var(--color-elevated)]/60 p-3 text-center">
                <p className="text-lg font-bold text-[var(--color-text-2)]">{result.totalOwned - result.matched}</p>
                <p className="text-[11px] text-[var(--color-text-dim)]">Not in DB</p>
              </div>
            </div>
            ...
            {result.games.length > 0 && (
              <div>
                <h4 className="mb-2 text-xs font-medium text-[var(--color-text-2)]">
                  Matched Games ({result.matched})
                </h4>
                <div className="max-h-80 overflow-y-auto rounded-lg border border-[var(--color-elevated)] divide-y divide-[var(--color-elevated)]/80">
                  {gamesToShow.map((game) => (
                    <a
                      key={game.id}
                      href={`/games/${game.slug}`}
                      className="flex items-center gap-3 px-3 py-2 hover:bg-[var(--color-elevated)]/60 transition-colors"
                    >
                      {game.headerImage ? (
                        <img
                        ...
                        <div className="flex h-8 w-14 flex-shrink-0 items-center justify-center rounded bg-[var(--color-elevated)]">
                          <svg
                ...
                {hasMore && (
                  <button
                    ...
                    className="mt-2 w-full rounded-lg border border-[var(--color-elevated)] py-1.5 text-xs text-[var(--color-text-2)] hover:border-[var(--color-border)] hover:text-gray-300 transition-colors"
                  >
                    {showAllGames
                      ? 'Show less'
            ...
            {result.matched === 0 && (
              <div className="rounded-lg border border-[var(--color-border)]/50 bg-[var(--color-elevated)]/40 px-3 py-3 text-center">
            ...
            <button
              onClick={handleReset}
              className="w-full rounded-lg border border-[var(--color-border)] py-2 text-xs font-medium text-[var(--color-text-2)] hover:border-[var(--color-text-dim)] hover:text-gray-300 transition-colors"
            >
