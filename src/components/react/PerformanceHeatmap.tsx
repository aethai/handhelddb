    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {stats && (
          <div className="flex flex-wrap gap-2">
            {stats.excellent > 0 && (
              <span className="inline-flex items-center gap-1 rounded-full bg-[#D4FF00]/10 border border-[#D4FF00]/20 px-2.5 py-1 text-xs text-[#D4FF00]">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1">
          <svg
            className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--color-text-dim)]"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            viewBox="0 0 24 24"
          ...
          <input
            ...
            className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] pl-10 pr-4 py-2 text-sm text-white placeholder-[var(--color-text-dim)] focus:border-[#D4FF00] focus:outline-none focus:ring-1 focus:ring-[#D4FF00]"
          />
        </div>
        ...
        <select
          ...
          className="rounded-lg border border-[var(--color-border)] bg-[var(--color-elevated)] px-3 py-2 text-sm text-gray-300 focus:border-[#D4FF00] focus:outline-none focus:ring-1 focus:ring-[#D4FF00] cursor-pointer"
        >
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[var(--color-text-dim)]">
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-sm bg-[#D4FF00]" />
          60+ FPS
        </span>
        <span className="flex items-center gap-1.5">
      ...
      {filteredAndSorted.length > 0 ? (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-white truncate group-hover:text-[#e8ff4d] transition-colors">
                      {entry.gameName}
                    </p>
                    <div className="mt-1 flex items-center gap-2 text-xs text-[var(--color-text-2)]">
                      {entry.preset && (
                ...
                <div className="mt-2 h-1 bg-[var(--color-elevated)] rounded-full overflow-hidden">
                  <div
        ...
        <div className="rounded-lg border border-dashed border-[var(--color-border)] bg-[var(--color-raised)]/50 p-8 text-center">
          ...
          <p className="text-[var(--color-text-2)] text-sm">No games match "{search}"</p>
          <button
            onClick={() => setSearch('')}
            className="mt-2 text-xs text-[#D4FF00] hover:text-[#e8ff4d] transition-colors"
          >
            Clear search
          </button>
        </div>
