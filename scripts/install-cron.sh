...
if [[ "${1:-}" == "--remove" ]]; then
    ...
    crontab -l 2>/dev/null | grep -v "${MARKER}" | grep -v "cron-sync-steam" | grep -v "cron-sync-protondb" | grep -v "cron-reindex-meilisearch" | grep -v "cron-consensus" | grep -v "cron-ingest-news" | grep -v "cron-generate-articles" | crontab -
...
CLEAN_CRON=$(echo "${EXISTING_CRON}" | grep -v "${MARKER}" | grep -v "cron-sync-steam" | grep -v "cron-sync-protondb" | grep -v "cron-reindex-meilisearch" | grep -v "cron-consensus" | grep -v "cron-ingest-news" | grep -v "cron-generate-articles" || true)
